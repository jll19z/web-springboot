...
public class VacationController {
    ...
    public Result getStaff(@PathVariable String id) {
        Vacation byId = vacationService.getById(id);
        return Result.ok().data("vacation", byId);
    }
    ...
    public Result pageCondition(@ApiParam(name = "page", value = "当前页码", required = true)@PathVariable Long page,
                                ...
                                @RequestBody(required = false) VacationQuery vacationQuery){//通过封装TeacherQuery对象来直接传递查询条件
        //创建分页page对象
        Page<Vacation> pageParam = new Page<>(page, limit);

        //调用方法实现多条件分页查询
        vacationService.pageQuery(pageParam,vacationQuery);

        //获取查询到的数据
        List<Vacation> records = pageParam.getRecords();
    ...
    public Result updateById(@RequestBody Vacation vacation){
        boolean flag = vacationService.updateById(vacation);
        if (flag){
            return Result.ok();
        }else {
            return Result.error();
